...
export const validateMyRestaurantRequest = [
  ...
  body("estimatedDeliveryTime")
    .isInt({ min: 0 })
    .withMessage("Estimated delivery time must be a positive integer number!"),
  body("cuisines")
    .isArray()
    .withMessage("Cuisines must be an array type!")
    .not()
    .isEmpty()
    .withMessage("Cuisines array can't be empty!"),
  body("menuItems").isArray().withMessage("Menu items must be an array type!"),
